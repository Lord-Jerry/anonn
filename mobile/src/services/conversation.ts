...
export default class ConversationService extends BaseService {
  ...
  async getAllConversations(conversationCursor?: Date, cursorType?: 'after' | 'before') {
    try {
  ...
  async startConversation({id, content}: {id: string; content: string}) {
    try {
      const {data} = await this.api.post<{conversationId: string}>(`/conversation/init-conversation/${id}`, {
        content: content,
      });
      return data;
    } catch (error) {
  ...
  async getConversationMessages(conversationId: string, messageCursor?: Date, cursorType?: 'after' | 'before') {
    try {
      const {data} = await this.api.get<IMessage[]>(`/messages/v2/${conversationId}`, {
        params: {
          cursorType,
          messageCursor,
        },
  ...
  async markConversationAsRead(conversationPublicId: string, messagePublicId: string) {
    try {
